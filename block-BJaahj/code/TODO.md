...
function multiMap(arrVals, arrCallbacks) {
  let obj = {}
  for(i = 0; i < arrCallbacks.length; i++){
    arrVals.forEach(arr => {
      obj[arr] = arrCallbacks.map(arr1 => arr1(arr))
    })
  }
  return obj
}
...
function objOfMatchesWithArray(array1, array2, callback) {
  let array3 = (array1.map(arr => {
    return callback.reduce((acc,cv) => cv(acc), arr)
  }))
  let obj = {};
  for(let i = 0; i < array2.length; i++){
      if(array3[i] === array2[i]){
          obj[array3[i]] = array2[i];
      }
  }
  return obj;
}
...
function objOfMatchesWithArray(array1, array2, callback) {
  ...
  for(i = 0; i < array2.length; i++){
    array1.forEach(arr => {
      obj[arr] = array2.map(arr1 => arr1(arr))
    })
  }
  return obj
}
...
```js
function schedule(arrayFun, arraySec){
  if(arrayFun.length === arraySec.length){
      for(let i = 0; i < arrayFun.length; i++){
        setTimeout(arrayFun[i], arraySec[i]*1000)
      }
    }
  else{
    alert(`invalid input`)
